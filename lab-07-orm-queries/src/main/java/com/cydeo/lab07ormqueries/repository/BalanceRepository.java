...
public interface BalanceRepository extends JpaRepository<Balance, Long> {
    ...
    boolean existsBalanceByCustomerId(Customer customer);
    //or:
   ...
   @Query(value = "SELECT distinct * FROM balanceWHERE amount IS NOT NULL ORDER BY amount DESC LIMIT 5", nativeQuery = true)
   // @Query(value="SELECT * FROM balance ORDER BY amount DESC LIMIT 5", nativeQuery = true)
    ...
    List<Balance> retrieveBalanceLessThanAmount(@Param("amount") BigDecimal amount); // @Param - it is not mandatory to put it, but it is for security reason
}